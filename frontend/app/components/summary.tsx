...
export default function Summary(props: SummaryProps) {
  ...
  return (
    <div>
      ...
      <p className="text-white select-text">{props.summary.summary}</p>
      {props.summary.sections_timestamps ? (
        <div className="space-y-3">
          ...
          <table className="w-full table-auto border-separate border-spacing-2 border-2 border-rose-600 rounded-lg">
            ...
            <tbody>
              {props.summary.sections_timestamps.map((section_timestamp, index) => (
                <tr key={index}>
                  <td className="px-2 py-2 flex border-2 border-teal-500 rounded-lg">
                    <span className="hover:text-rose-600 cursor-pointer" onClick={() => props.handleSeekTo(section_timestamp.timestamps[0])}>
                      {convertSecondsToMinutesAndSeconds(section_timestamp.timestamps[0])}
                    </span>
                    -
                    <span className="hover:text-rose-600 cursor-pointer" onClick={() => props.handleSeekTo(section_timestamp.timestamps[1])}>
